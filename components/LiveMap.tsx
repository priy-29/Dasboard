...
export default function LiveMap({points,zoom=15}:{points:Point[];zoom?:number}){
 ...
 const route=points.filter(p=>p.label==="Rumah makan"||p.label.startsWith("Kurir")||p.label==="Pelanggan");
 return <div className="relative h-72 w-full overflow-hidden bg-zinc-800">{tiles.map((t,i)=><img key={i} src={`https://tile.openstreetmap.org/${zoom}/${t.x}/${t.y}.png`} alt="" className="absolute h-64 w-64 max-w-none select-none" style={{left:`calc(50% + ${t.x*TILE_SIZE-c.x}px)`,top:`calc(50% + ${t.y*TILE_SIZE-c.y}px)`}} draggable={false}/>)}
 {route.length>=2&&<svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 1000 500" preserveAspectRatio="none"><polyline points={route.map(p=>{const q=world(p.lat,p.lng,zoom);return `${500+(q.x-c.x)*1000/500},${250+(q.y-c.y)*500/250}`}).join(" ")} fill="none" stroke="white" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" opacity="0.9"/><polyline points={route.map(p=>{const q=world(p.lat,p.lng,zoom);return `${500+(q.x-c.x)*1000/500},${250+(q.y-c.y)*500/250}`}).join(" ")} fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-500"/></svg>}
 {points.map((p,i)=>{const q=world(p.lat,p.lng,zoom);return <div key={i} className="absolute -translate-x-1/2 -translate-y-full text-center" style={{left:`calc(50% + ${q.x-c.x}px)`,top:`calc(50% + ${q.y-c.y}px)`}}><div className={`mx-auto flex h-10 w-10 items-center justify-center rounded-full border-2 border-white text-lg shadow-xl ${p.className}`}>{p.emoji}</div><span className="mt-1 inline-block whitespace-nowrap rounded-md bg-zinc-950/90 px-2 py-1 text-[10px] font-semibold text-white shadow">{p.label}</span></div>})}<div className="absolute bottom-2 right-2 rounded bg-white/90 px-2 py-1 text-[9px] text-zinc-700">© OpenStreetMap contributors</div></div>;
}
