...
export default function PublicNav() {
  ...
  return <nav aria-label="Navigasi pelanggan">
    <div className="fixed left-0 top-0 z-30 hidden h-screen w-[230px] border-r border-zinc-200/80 bg-white/90 p-5 shadow-[8px_0_40px_rgba(0,0,0,.04)] backdrop-blur-xl lg:flex lg:flex-col">
    ...
    <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-zinc-700 bg-zinc-950 px-3 pb-[max(10px,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_30px_rgba(0,0,0,.45)] backdrop-blur-xl lg:hidden"><div className="mx-auto grid max-w-md grid-cols-3 gap-1">{links.map(({href,label,icon:Icon}) => <a key={href} href={href} className={`flex flex-col items-center gap-1 rounded-xl py-2 text-[10px] font-bold ${active(href)?"bg-zinc-900 text-emerald-400":"text-zinc-500"}`}><Icon size={20}/>{label}</a>)}</div></div>
  </nav>;
}
