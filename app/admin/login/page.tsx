...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.user.email?.toLowerCase() === ADMIN_EMAIL) { router.replace("/admin/pos"); return; }
  ...
  async function login(e: FormEvent) {
    ...
    router.replace("/admin/pos"); router.refresh();
  }
  ...
  return <main className="admin-login-page relative flex min-h-screen items-center justify-center overflow-hidden bg-[#07090d] p-5 text-white">
    ...
    <div className="relative w-full max-w-md overflow-hidden rounded-[30px] border border-white/10 bg-[#101419]/95 p-6 shadow-[0_30px_90px_rgba(0,0,0,.55)] backdrop-blur-2xl sm:p-8">
      <div className="mb-7 flex items-start gap-4">
        ...
        <div><p className="text-[10px] font-black uppercase tracking-[.22em] text-emerald-400">Dasboard · Admin POS</p><h1 className="mt-1 text-2xl font-black tracking-tight">Masuk ke Kasir</h1><p className="mt-1 text-sm text-zinc-500">Kelola penjualan, menu, struk, ongkir, dan kurir.</p></div>
      </div>
      <form onSubmit={login} className="space-y-4">
        ...
        <button disabled={loading} className="h-12 w-full rounded-2xl bg-emerald-400 font-black text-[#06110c] shadow-lg shadow-emerald-500/10 transition hover:bg-emerald-300 disabled:cursor-wait disabled:opacity-50">{loading ? "Memeriksa akun..." : "Masuk ke Kasir"}</button>
      </form>
