...
export default function CourierPage() {
  ...
  async function startTracking() {
    if (
      ...
      order.delivery_arrival_requested_at
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.user.email?.toLowerCase() === ADMIN_EMAIL) {
        ...
        await loadTasks();
        if (!cancelled) {
          interval = window.setInterval(() => void loadTasks(), 5000);
        }
        return;
      }
  ...
  const waiting = active.filter((item) => item.delivery_arrival_requested_at);
  ...
  return (
    <main className="min-h-screen bg-zinc-950 p-4 text-white md:p-6">
      <div className="mx-auto max-w-xl">
        ...
        <div className="rounded-3xl border border-zinc-800 bg-zinc-900/80 p-5 text-center shadow-xl">
          <div
            className={`mx-auto flex h-16 w-16 items-center justify-center rounded-2xl ${
              isAdmin ? "bg-blue-500/10" : "bg-orange-500/10"
            } text-4xl`}
          >
            🛵
          </div>
          <div
            className={`mt-3 inline-flex rounded-full px-3 py-1 text-[10px] font-black ${
              isAdmin ? "bg-blue-950 text-blue-300" : "bg-orange-950 text-orange-300"
            }`}
          >
          ...
          <p className="mt-1 text-sm text-zinc-500">
            {isAdmin
              ? "Pantau semua order delivery dan posisi kurir."
              : "Pesanan delivery untuk akun kurir ini."}
          </p>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl border border-zinc-800 bg-zinc-900 p-2">
          <button
            ...
            className={`rounded-xl py-3 text-sm font-bold ${
              tab === "ready" ? "bg-emerald-500 text-zinc-950" : "text-zinc-400"
            }`}
          >
          ...
          <button
            ...
            className={`rounded-xl py-3 text-sm font-bold ${
              tab === "active" ? "bg-orange-500 text-zinc-950" : "text-zinc-400"
            }`}
          >
        ...
        <section className="mt-3 rounded-3xl border border-zinc-800 bg-zinc-900 p-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold">
                {tab === "ready" ? "📦 Order Siap Diambil" : "🛵 Order Aktif"}
              </h2>
          ...
          {loading ? (
            <div className="mt-4 space-y-2">
              {[1, 2, 3].map((item) => (
                <div key={item} className="h-20 animate-pulse rounded-2xl bg-zinc-800/60" />
              ))}
            </div>
          ) : shown.length === 0 ? (
            <div className="py-10 text-center">
              ...
              <p className="mt-2 font-semibold">
                {tab === "ready"
                  ...
                  : "Belum ada order yang sedang diantar."}
              </p>
              <p className="mt-1 text-xs text-zinc-500">
                Halaman akan mengecek lagi otomatis.
              </p>
            </div>
          ) : (
            <div className="mt-4 space-y-2">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 text-xs text-orange-300">
                      {item.status === "processing"
                        ? "📦 Siap diambil"
                        : item.delivery_arrival_requested_at
                          ? "⏳ Menunggu"
        ...
        {tab === "active" && waiting.length > 0 && (
          <p className="mt-3 rounded-2xl border border-blue-900 bg-blue-950/30 p-4 text-xs text-blue-200">
            🤝 {waiting.length} order sudah dinyatakan sampai dan sedang menunggu persetujuan pembeli.
          </p>
        )}

        {order && (
          <section className="mt-5 overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900 shadow-xl">
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-bold text-emerald-400">
                    {order.status === "processing" ? "SIAP DIAMBIL" : "PENGANTARAN AKTIF"} · #{order.id.slice(0, 8)}
                  </p>
                  <h2 className="mt-1 text-xl font-black">
                    {order.status === "processing"
                      ? `Ambil untuk ${order.customer_name}`
                      : `Antar ke ${order.customer_name}`}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">{order.customer_address}</p>
                ...
                >
                  {tracking ? "● GPS LIVE" : waitingApproval ? "⏳ MENUNGGU" : "📦 SIAP"}
                </span>
              </div>

              {order.status === "processing" ? (
                <div className="mt-4 rounded-2xl border border-emerald-900/50 bg-emerald-950/20 p-4 text-sm text-emerald-200">
                  {isAdmin
                    ? "👀 Admin dapat memantau order ini. Pengambilan hanya dilakukan oleh akun kurir."
                    : "📦 Pesanan sudah diproses dan siap diambil kurir."}
                </div>
              ) : (
                <div className="mt-4 rounded-2xl border border-blue-900/50 bg-blue-950/20 p-3 text-xs text-blue-200">
                  {waitingApproval
                    ? "🤝 Kurir sudah tiba. Menunggu pembeli menekan tombol persetujuan penerimaan."
                    : tracking
                      ? "📍 GPS asli HP kurir sedang dikirim ke server."
                      : isAdmin
                        ? "👀 Admin sedang memantau. GPS berasal dari HP kurir yang aktif."
            ...
            <div className="p-5">
              {isAdmin ? (
                ...
                <button
                  onClick={claimTask}
                  disabled={claiming}
                  className="w-full rounded-xl bg-emerald-500 py-3 text-sm font-bold text-zinc-950 disabled:opacity-50"
                >
                ...
                <>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`rounded-xl py-3 text-sm font-bold ${
                        tracking ? "bg-red-500 text-white" : "bg-emerald-500 text-zinc-950"
                      } disabled:opacity-50`}
                    >
                  ...
                  <button
                    ...
                    disabled={waitingApproval || requesting}
                    className="mt-3 w-full rounded-xl bg-blue-500 py-3 text-sm font-bold disabled:opacity-50"
                  >
                    {requesting
                      ? "⏳ Mengirim..."
                      : waitingApproval
                        ? "⏳ Menunggu persetujuan pembeli"
                        : "🤝 Minta Persetujuan Pembeli"}
                  </button>
                </>
              )}
