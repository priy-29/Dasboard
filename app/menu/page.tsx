...
type Product = { id: string; name: string; category: string; price: number; emoji: string; description: string | null; stock: number; is_available: boolean };
...
export default function MenuPage() {
  ...
  useEffect(() => { (async () => { const { data, error } = await supabase.from("products").select("id,name,category,price,emoji,description,stock,is_available").eq("is_available", true).order("created_at"); if (error) setError(error.message); else setProducts((data || []) as Product[]); setLoading(false); })(); }, []);
