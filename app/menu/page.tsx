...
export default function MenuPage() {
  ...
  return <main className="min-h-screen bg-zinc-950 pb-28 text-white">
    ...
    {preview && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setPreview(null)}><div className="relative max-h-[90vh] max-w-3xl" onClick={e => e.stopPropagation()}><button type="button" onClick={() => setPreview(null)} className="absolute -right-2 -top-12 rounded-full bg-zinc-800 px-4 py-2 text-white">✕</button><img src={preview.image_url!} alt={preview.name} className="max-h-[85vh] w-auto max-w-full rounded-2xl object-contain shadow-2xl" /><p className="mt-3 text-center font-semibold text-white">{preview.name}</p></div></div>}
  </main>;
}
