...
export default function CheckoutPage() {
  ...
  function requestLocation() {
    setError("");
    if (!navigator.geolocation) { setError("Browser ini tidak mendukung lokasi. Silakan masukkan alamat secara manual."); return; }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      err => {
        setLocating(false);
        if (err.code === 1) setError("Izin lokasi ditolak. Silakan izinkan lokasi untuk situs ini agar pesanan bisa diantar.");
        else if (err.code === 2) setError("Lokasi tidak ditemukan. Pastikan GPS/lokasi HP aktif lalu coba lagi.");
        else setError("Gagal mengambil lokasi. Coba tekan tombol lokasi lagi.");
      },
      { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 }
    );
  }

  function chooseMethod(method: "delivery" | "pickup") {
    setDeliveryMethod(method);
    setError("");
    if (method === "pickup") setCoords(null);
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (deliveryMethod === "delivery" && !coords) { setError("Tekan tombol 'Izinkan & ambil lokasi saya' terlebih dahulu."); return; }
