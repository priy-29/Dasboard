-- Canonical order creation contract. Customer payment status is never trusted as paid;
-- only the authenticated admin/POS flow may create a paid transaction.
create or replace function public.create_order_with_items(
  ...
  p_items jsonb,
  p_delivery_method text default 'delivery'::text,
  p_customer_lat double precision default null::double precision,
  p_customer_lng double precision default null::double precision,
...
declare
  ...
  v_distance_km double precision := null;
  v_item jsonb;
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if v_qty > 100 then raise exception 'Jumlah produk per item terlalu banyak'; end if;
  ...
  if v_type='delivery' then
    ...
    v_lat:=radians(p_customer_lat-v_setting.restaurant_lat);
    v_lng:=radians(p_customer_lng-v_setting.restaurant_lng);
    ...
    if v_subtotal>=v_setting.free_shipping_minimum then v_delivery_fee:=0;
    else v_delivery_fee:=v_setting.base_fee+ceil(v_distance_km)*v_setting.per_km_fee; end if;
  end if;
  ...
  values(trim(p_customer_name),trim(p_customer_phone),case when v_type='delivery' then trim(p_customer_address) when v_type='dine_in' then 'Makan di tempat' else 'Ambil sendiri' end,nullif(trim(coalesce(p_notes,'')),''),p_payment_method,case when v_type='delivery' then 'delivery' else 'pickup' end,v_type,case when v_type='takeaway' and p_order_type<>'pickup' then nullif(trim(p_pickup_time),'') else null end,nullif(trim(coalesce(p_payment_note,'')),''),v_payment_status,case when v_type='delivery' then p_customer_lat else null end,case when v_type='delivery' then p_customer_lng else null end,case when v_type='delivery' then v_distance_km else null end,v_subtotal,case when v_type='delivery' then v_delivery_fee else 0 end,v_subtotal+case when v_type='delivery' then v_delivery_fee else 0 end,'pending')
  returning id into v_order_id;

  for v_item in select * from jsonb_array_elements(p_items) loop
    select * into v_product from public.products where id=(v_item->>'product_id')::uuid and is_available=true;
    v_qty:=(v_item->>'quantity')::integer;
    insert into public.order_items(order_id,product_id,product_name,price,quantity,notes,options,special_note)
    values(v_order_id,v_product.id,v_product.name,v_product.price,v_qty,nullif(trim(coalesce(v_item->>'notes','')),''),coalesce(v_item->'options','{}'::jsonb),nullif(trim(coalesce(v_item->>'special_note','')),''));
...
create or replace function public.create_order_with_items_v2(
  p_customer_name text,p_customer_phone text,p_customer_address text,p_notes text,p_payment_method text,p_items jsonb,
  p_delivery_method text default 'delivery'::text,p_customer_lat double precision default null::double precision,p_customer_lng double precision default null::double precision,
  p_order_type text default null::text,p_pickup_time text default null::text,p_payment_note text default null::text,p_payment_status text default 'unpaid'::text)
returns jsonb
language plpgsql security definer set search_path=public,extensions
as $$
declare v_order_id uuid; v_result jsonb;
begin
  v_order_id:=public.create_order_with_items(p_customer_name,p_customer_phone,p_customer_address,p_notes,p_payment_method,p_items,p_delivery_method,p_customer_lat,p_customer_lng,p_order_type,p_pickup_time,p_payment_note,p_payment_status);
  select jsonb_build_object('id',id,'order_code',order_code,'order_type',order_type,'pickup_time',pickup_time,'payment_method',payment_method,'payment_note',payment_note,'payment_status',payment_status,'delivery_fee',delivery_fee,'delivery_distance_km',delivery_distance_km,'total',total) into v_result from public.orders where id=v_order_id;
  return v_result;
end;
$$;
revoke all on function public.create_order_with_items_v2(text,text,text,text,text,jsonb,text,double precision,double precision,text,text,text,text) from public;
grant execute on function public.create_order_with_items_v2(text,text,text,text,text,jsonb,text,double precision,double precision,text,text,text,text) to anon, authenticated;
